...
### 1️⃣ 카테고리 기반 민원 분류
- **7개 카테고리**: 교통/주차, 환경/위생, 안전/치안, 공원/시설, 동물, 일반행정, 기타
- **시각적 구분**: 각 카테고리별 색상 코드 배지
- **필터링**: 카테고리별로 민원 조회 가능
- **통계 분석**: 지역별 카테고리 분포 확인

### 2️⃣ 위치 기반 민원 제보
- **네이버 지도 연동**: 정확한 위치에 민원을 등록
  - 민원 작성: Interactive 모드 (탭하여 위치 선택)
  - 민원 상세: Read-only 모드 (위치 확인만)
- **사진 업로드**: 갤러리 또는 카메라에서 최대 10개 이미지
  - 지원 형식: JPG, JPEG, PNG, WEBP
  - 이미지 미리보기 및 순서 표시
  - 개별 이미지 삭제 가능
- **GPS 자동 감지**: 현재 위치 자동 설정
...
| 기술 | 버전 | 용도 |
|------|------|------|
| **Flutter** | ^3.10.4 | 크로스 플랫폼 앱 프레임워크 |
| **Riverpod** | ^2.6.1 | 상태 관리 (code generation) |
| **네이버 지도 SDK** | ^1.4.4 | 지도 표시 및 위치 선택 |
| **Dio** | ^5.7.0 | HTTP 클라이언트 |
| **Retrofit** | ^4.4.1 | 타입 안전한 API 클라이언트 |
| **GoRouter** | ^14.6.2 | 선언적 라우팅 |
| **Image Picker** | ^1.1.2 | 갤러리/카메라 사진 선택 |
| **Geolocator** | ^13.0.2 | GPS 위치 추적 |
| **Cached Network Image** | ^3.4.1 | 이미지 캐싱 |
| **Flutter Secure Storage** | ^9.2.2 | 안전한 데이터 저장 |
| **Dartz** | ^0.10.1 | Either 패턴 |
...
| **PostgreSQL + PostGIS** | - | 지리 정보 데이터베이스 |
| **JWT (simplejwt)** | 5.3.1 | JWT 인증 시스템 |
...
### Phase 2: 이미지 업로드 및 카테고리 (2025년 1월 - 완료)

#### ✅ 완료된 기능
- ✅ 7개 카테고리 시스템 (traffic, env, safety, facility, animal, admin, etc)
- ✅ 카테고리 드롭다운 UI (민원 작성)
- ✅ 카테고리 배지 표시 (민원 카드, 상세 페이지)
- ✅ `image_picker` 패키지 통합
- ✅ 갤러리에서 여러 이미지 선택 (최대 10개)
- ✅ 카메라로 직접 촬영
- ✅ 이미지 미리보기 및 삭제 UI
...
- 좌표 → 주소 변환이 작동하지 않음
- 민원 목록에 "강남구" 대신 "37.4979, 127.0276" 형태로 표시됨

**영향**:
- 사용자 경험 저하 (좌표 대신 지역명을 보고 싶어함)
- 기능적으로는 정상 작동 (민원 등록/조회 가능)

**원인 조사 중**:
- Naver Cloud Platform 설정 확인 완료
- API 키 유효성 확인 완료
- 구독 상태 확인 필요
...
- 호스트 PC의 서버에 접근하려면 **10.0.2.2** 사용 필요
- 앱은 자동으로 플랫폼을 감지하여 올바른 URL 사용:
  ```
...
- `android:usesCleartextTraffic="true"` (AndroidManifest.xml Line 17)
- 개발 환경에서 HTTP 통신 허용
- 프로덕션에서는 HTTPS 필수
